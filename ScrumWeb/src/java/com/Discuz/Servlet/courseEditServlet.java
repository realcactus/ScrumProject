...
public class courseEditServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet courseEditServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet courseEditServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
        ...
        String url=(String)session.getAttribute("url3");
           String idString=request.getParameter("id");
              ...
              boolean flag=dao.updateCourseDao(course);
                if(flag)
              {
                    response.sendRedirect("/Discuz/MyInfo/courseEdit.jsp");
              }
        processRequest(request, response);
    }
