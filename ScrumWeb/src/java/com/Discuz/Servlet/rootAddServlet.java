...
public class rootAddServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet rootAddServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet rootAddServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int judgeRrgister=dao.checkName(user);
             if(judgeRrgister==0)
            {
                response.sendRedirect("/Discuz/User/Account/rootAdd.jsp?error=yes");
            }
             else{
             boolean flag=dao.rootAddDao(user);
             if(flag)
            {
                   session.setAttribute("username", studentID);
                response.sendRedirect("/Discuz/User/StudentInfor/myInfo.jsp");
            }
             }
        } catch (SQLException ex) {
            Logger.getLogger(rootAddServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
        processRequest(request, response);
    }
