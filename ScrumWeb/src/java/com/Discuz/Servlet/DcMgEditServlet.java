 ...
 * @author OovEver
 */
public class DcMgEditServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet DcMgEditServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet DcMgEditServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
         ...
         User user=new User();
            ...
            user.setCredit(creditI);
             boolean flag=dao.updateRootDao(user);
               if(flag)
            {
                response.sendRedirect("/Discuz/MyInfo/DcManagerEdit.jsp?id='"+idString+"'");
            }
        processRequest(request, response);
    }
